...
export class EditAssignorUseCase {
  ...
  async execute({
    ...
    const assignorByPhone = await this.assignorsRepository.findByPhone(
      Raw.createFromText(phone).value,
    )
    ...
    const assignorByDocument = await this.assignorsRepository.findByDocument(
      Raw.createFromText(document).value,
    )
